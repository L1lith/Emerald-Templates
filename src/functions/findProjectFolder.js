...
async function findProjectFolder(name) {
  ...
  if (projectFolders.length < 1) console.warn(chalk.red("Warning: Found no template folders, try setting some up first"))
  ...
  const projectMap = {}
  await Promise.all(projectFolders.sort().map(async projectPath => {
