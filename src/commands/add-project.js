...
async function addProject(options) {
  let projectPath = options['add-project'][0] || (await askQuestion("Please enter the path to your project templates storage folder\n> ")).trim()
  ...
  if (Object.values(config.projectFolders).includes(projectFolder)) throw new Error("That folder has already been added")
  const projectConfig = getEmeraldConfig(projectFolder)
  let {name} = projectConfig
  if (projectFolders.includes(projectFolder)) throw new Error("That project folder has already been added")
